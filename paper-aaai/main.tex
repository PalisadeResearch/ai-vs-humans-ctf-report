...
\begin{links}
    \link{Analysis code and data}{redacted_link}
    \end{links}
...
Similarly, while a popular cyber benchmark InterCode-CTF \cite{yang2023language} reported up to 40\% AI success rates with GPT-4o, \cite{authors2024} achieved 92\% in just 5 weeks of elicitation.\footnote{While the paper's headline result is 95\%, we cite the ablation using GPT-4o only.}
...
\begin{tabular}{lcc}
\hline
\textbf{Agent} & \textbf{Challenges solved} & \textbf{Leaderboard rank}\footnotemark \\
\hline
CAI & 19 / 20 & 18 \\
EnIGMA & 19 / 20 & 23 \\
Claude Code CTF & 19 / 20 & 24 \\
React\&Plan & 19 / 20 & 31 \\
FCT & 17 / 20 & 55 \\
Aider & 11 / 20 & 115 \\
Project S1ngularity & 8 / 20 & 139 \\
\hline
\end{tabular}
\caption{AI agents standings for \emph{AI vs. Humans}\footnotemark}
\label{tab:ai_vs_hum_standings}
\end{table*}
\footnotetext[2]{In case of teams solving an equal number of challenges, the faster teams rank higher on the leaderboard.}
\footnotetext{The full leaderboard, including human teams, is available at REDACTED.}

The first event in our series was the \emph{AI vs. Humans} CTF, organized in collaboration with Hack The Box on 14-16 March 2025.\footnote{Event link: REDACTED} This event was the first to publicly pit fully autonomous AI agents against experienced human teams in real-time, offering 20 cybersecurity challenges over 48 hours. We offered a prize pool of \$7500 to incentivize participation and effort.
...
We calibrated the challenge difficulty based on preliminary evaluations of our React\&Plan agent \cite{authors2024} on older Hack The Box-style tasks such that the AI could solve $\sim$50\% of tasks.

See Appendix~\ref{sec:agent_designs} for the submitted agents' designs. All event performance data is available at [REDACTED].

\subsection{Absolute standings}

Overall 403 teams registered for the event, of which 158 solved at least 1 challenge: 152 human teams and 6 AI teams. The authors ran 2 AI agents during this event: a Claude Code adapted for CTFs and a React\&Plan agent.
...
\begin{table*}[htb]
    ...
    \hline
    CAI & 20 / 62 & 859 \\
    Authors' Claude Code\footnotemark & 5 / 62 & 2496 \\
    Authors' Aider & 3 / 62 & 2953 \\
    Authors' React\&Plan & 2 / 62 & 3199 \\
    \hline
    \end{tabular}
    \caption{AI agents standings for \emph{Cyber Apocalypse} CTF\footnotemark}
    \label{tab:ca_standings}
\end{table*}
\footnotetext{Authors' submissions performed poorly because our harness was not designed to interact with external machines, while about 2/3 of challenges required it.}
\footnotetext{The full leaderboard, including human teams, is available at https://ctftime.org/event/2674}
...
On 21--26 March 2025 we hosted an AI track at \emph{Cyber Apocalypse},\footnote{https://www.hackthebox.com/events/cyber-apocalypse-2025} an annual Hack The Box competition that attracts a large number of human participants and offers a diverse set of challenges. 8129 human teams (18369 players) registered for the event, and 3994 teams solved at least one challenge.
...
\item \textbf{For CTF Organizers}: Adding an AI track can increase visibility, attract participants, and introduce new research and media interest. The authors are happy to assist with this.
\end{itemize}
...
[Acknowledgements will be added upon publication to maintain anonymity during review.]
...
% Authors' contributions section removed for anonymous submission
...
Used a custom harness design they spent about 500 dev-hours on, see \cite{caiteampaper}.
...
\subsection{Authors' Claude Code}

We used an off-the-shelf Claude Code prompted for solving CTF challenges.

\subsection{Authors' React\&Plan}

See \cite{authors2024}.
...
We can measure the position of the human team on the leaderboard as a measure of its expertise. Figure~\ref{fig:ca_time_horizon_detailed} shows how the 50\%-task-completion time horizon estimates change depending on which percentile of the human teams we consider to be experts. The estimates maintain a similar order of magnitude.
...
Does this paper rely on one or more datasets? (yes, the scoreboards and flag submission logs from the CTF events)

If yes, please complete the list below.

    A motivation is given for why the experiments are conducted on the selected datasets (yes)

    All novel datasets introduced in this paper are included in a data appendix. (yes, all data is available in the github repo (no link yet for anonymity))
...
All source code required for analyzing the experiments will be made publicly available upon publication of the paper with a license that allows free usage for research purposes. (yes)

% References and End of Paper
% These lines must be placed at the end of your paper
\bibliographystyle{aaai2026}
\bibliography{../paper-typst/ref}
\end{document}